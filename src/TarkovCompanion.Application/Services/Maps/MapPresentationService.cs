    ...
    Uri LicenseUri)
{
    ...
    public IReadOnlyList<MapOverlayElement> VisibleOverlayElements => OverlayElements
...
public static class MapTilePlanner
{
    public static MapTilePlan Plan(MapVariant variant, int zoom, int maximumTiles = 64)
    {
        ...
        var count = (long)(maximumTileX - minimumTileX + 1) * (maximumTileY - minimumTileY + 1);
        ...
        for (var y = minimumTileY; y <= maximumTileY; y++)
        {
            for (var x = minimumTileX; x <= maximumTileX; x++)
            {
                tiles.Add(new(
                    zoom,
                    x,
        ...
        return new(
            tiles,
            (maximumTileX - minimumTileX + 1) * variant.TileSize,
            (maximumTileY - minimumTileY + 1) * variant.TileSize,
            minimumTileX * variant.TileSize,
            minimumTileY * variant.TileSize,
            null);
    }
}

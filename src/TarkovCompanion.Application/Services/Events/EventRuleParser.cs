...
public static class EventRuleText
{
    private const int VisibleEffectLimit = 4;

    public static string Preview(EventRuleSet rules) =>
        JoinBounded(rules.Effects.Select(Describe), "; ");

    public static string ActiveSummary(ActiveEventRules rules) =>
        JoinBounded(rules.Rules.Select(rule => Describe(rule.Effect)), " · ");
    ...
    private static string JoinBounded(IEnumerable<string> values, string separator)
    {
        ...
        var shown = string.Join(separator, distinct.Take(VisibleEffectLimit));
        return distinct.Length > VisibleEffectLimit
            ? $"{shown}{separator}+{distinct.Length - VisibleEffectLimit} more"
            : shown;
    }

    private static string Describe(EventRuleEffect effect) => effect switch
    {
        TraderPriceMultiplierRule rule => $"{rule.TraderName} prices x{rule.Multiplier:0.##}",
        FleaAvailabilityRule { Enabled: true } => "Flea open",
        FleaAvailabilityRule { Enabled: false } => "Flea closed",
        MapAvailabilityRule rule => $"{rule.MapName} {(rule.Available ? "open" : "closed")}",
        BossSpawnMultiplierRule rule => $"{rule.BossName} spawns x{rule.Multiplier:0.##}",
    ...
    {
        ({ } start, { } end) => $"{rule.QuestName} available {LocalTime.Date(start)}–{LocalTime.Date(end)}",
        ({ } start, null) => $"{rule.QuestName} available from {LocalTime.Date(start)}",
        (null, { } end) => $"{rule.QuestName} available until {LocalTime.Date(end)}",
        _ => $"{rule.QuestName} availability unchanged",
    };
}

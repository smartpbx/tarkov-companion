    ...
    TimeProvider? timeProvider = null)
{
    ...
    public Task<MapAssetCacheResult> GetSvgAsync(MapVariant variant, CancellationToken cancellationToken)
    {
        ...
        return variant.SvgPath is null
            ? Task.FromResult(new MapAssetCacheResult(null, "This variant has no SVG asset."))
            : GetAsync(variant.SvgPath, variant.Author, variant.AuthorLink, cancellationToken);
    }
    ...
    private void ValidateOptions()
    {
        ...
        if (options.RefreshAfter < TimeSpan.Zero || options.RequestTimeout <= TimeSpan.Zero || options.MaximumAssetBytes <= 0)
        {

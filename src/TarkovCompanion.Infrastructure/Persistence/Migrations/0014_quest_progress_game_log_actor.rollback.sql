-- Restore the pre-GameLog actor CHECK. Rows written as GameLog cannot fit that constraint and
-- are dropped on purpose: a downgrade that kept them would fail the rebuild entirely.

CREATE TABLE quest_progress_journal_v1 (
...
INSERT INTO quest_progress_journal_v1(
    id,
    profile_id,
    game_mode,
    generation,
...
WHERE actor IN ('User', 'Import', 'SystemMigration');

DROP TABLE quest_progress_journal;
ALTER TABLE quest_progress_journal_v1 RENAME TO quest_progress_journal;

...
///
/// [#902 P6] Setup went from fourteen sections to eight and every entry moved with its row.
/// <c>V2SettingsIndexLandingTests</c> hosts the real Setup view, opens each entry's section and
/// fails unless its target is on screen there: two entries had pointed at controls that were
/// hidden in the running app (Report a problem's button without the review panel, and the self-test's
/// "previous run" line, which only shows after a crash).
/// </remarks>
public static class V2SettingsIndex
{
    ...
    [
        // Game & Capture
        Setup("screenshot-folder", "V2.Shell.Setting.ScreenshotFolder", V2SetupSection.GameCapture, "v2-setup-screenshot-folder", "screenshots", "folder", "path"),
        Setup("log-folder", "V2.Shell.Setting.LogFolder", V2SetupSection.GameCapture, "v2-setup-log-folder", "logs", "folder", "path"),
        Setup("capture-shortcut", "V2.Shell.Setting.CaptureShortcut", V2SetupSection.GameCapture, "v2-setup-capture-shortcut", "alt+shift+c", "hotkey", "capture"),
        Setup("screenshot-cleanup", "V2.Shell.Setting.ScreenshotCleanup", V2SetupSection.GameCapture, "v2-setup-choose-retention", "tidy", "retention", "delete screenshots", "privacy"),
        Setup("loot-return", "V2.Shell.Setting.LootReturn", V2SetupSection.GameCapture, "v2-setup-loot-return", "loot scan", "back to map", "seconds"),
        Setup("loot-tablet-only", "V2.Shell.Setting.LootTabletOnly", V2SetupSection.GameCapture, "v2-setup-loot-tablet-only", "loot scan", "tablet"),
        ...
        Setup("move-progress", "V2.Shell.Setting.MoveProgress", V2SetupSection.ProfileProgress, "v2-setup-profile-transfer", "export", "import", "backup", "json"),
        Setup("tarkovtracker", "V2.Shell.Setting.TarkovTracker", V2SetupSection.ProfileProgress, "v2-setup-tracker-token", "tracker", "token"),
        ...
        // Appearance & Window
        Setup("theme", "V2.Shell.Setting.Theme", V2SetupSection.AppearanceWindow, "v2-setup-appearance-themes", "dark", "light", "appearance", "colour", "color"),
        Setup("colour-vision", "V2.Shell.Setting.ColourVision", V2SetupSection.AppearanceWindow, "v2-setup-appearance-visions", "colour blind", "color blind", "deuteranopia", "protanopia", "accessibility"),
        Setup("text-size", "V2.Shell.Setting.TextSize", V2SetupSection.AppearanceWindow, "v2-setup-appearance-textscales", "font", "bigger text", "accessibility"),
        ...
        Setup("displays", "V2.Shell.Setting.Displays", V2SetupSection.AppearanceWindow, "v2-setup-displays-refresh", "monitor", "screen", "window"),

        // Data & Network
        Setup("local-only", "V2.Shell.Setting.LocalOnly", V2SetupSection.DataNetwork, "v2-setup-network-local-only", "offline", "network", "privacy", "internet"),
        Setup("squad-sharing", "V2.Shell.Setting.SquadSharing", V2SetupSection.DataNetwork, "v2-setup-network-SquadSharing-switch", "network", "relay", "team", "group"),
        Setup("update-checks", "V2.Shell.Setting.UpdateChecks", V2SetupSection.DataNetwork, "v2-setup-network-UpdateChecks-switch", "network", "updates"),
        Setup("problem-reports", "V2.Shell.Setting.ProblemReports", V2SetupSection.DataNetwork, "v2-setup-network-ProblemReports-switch", "network", "bug"),
        Setup("tracker-network", "V2.Shell.Setting.TrackerNetwork", V2SetupSection.DataNetwork, "v2-setup-network-TarkovTracker-switch", "network", "tracker"),
        Setup("game-data", "V2.Shell.Setting.GameData", V2SetupSection.DataNetwork, "v2-setup-sync", "sync", "tarkov.dev", "refresh", "catalog"),
        Setup("squad", "V2.Shell.Setting.Squad", V2SetupSection.DataNetwork, "v2-setup-open-team", "team", "group", "share"),

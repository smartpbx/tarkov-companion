...
public static class AppComposition
{
    ...
    public static ServiceProvider Build(AppCommandLine commandLine, AppCompositionSettings? settings = null)
    {
        ...
        services.AddSingleton<ICaptureSessionService>(provider => new CaptureSessionCoordinator(
            ...
            stageTimeline: provider.GetService<ICaptureStageTimeline>()));
        ...
        services.AddSingleton<ProfileModeFollower>(); // [#712 decision 4] the profile follows the game's mode
        services.AddSingleton(provider => new SetupProfilesViewModel(
            ...
            new(provider.GetRequiredService<ProfileModeFollower>(), action => Avalonia.Threading.Dispatcher.UIThread.Post(action))));

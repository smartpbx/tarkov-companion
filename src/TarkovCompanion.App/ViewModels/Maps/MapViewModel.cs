...
public sealed class MapViewModel : INotifyPropertyChanged, IDisposable
{
    ...
    public async Task SelectFloorAsync(MapFloorDefinition floor)
    {
        ...
        SelectedFloor = floor;
        if (_renderModel is not null)
        {
            _renderModel = _renderModel.SelectFloor(floor.Id);
        }

        if (SelectedVariant is { } variant && floor.TilePath is not null)
        {
            await LoadTilesAsync(variant with { TilePath = floor.TilePath }, _lifetime.Token).ConfigureAwait(true);
        }
    }
    ...
    private async Task LoadVariantAsync(MapVariant variant, bool persist)
    {
        ...
        try
        {
            ...
            {
                var source = variant.SvgPath;
                var cached = source is null
                    ? new MapAssetCacheResult(null, "No upstream artwork is configured for this variant.")
                    : await _assetCache.GetAsync(source, variant.Author, variant.AuthorLink, cancellationToken).ConfigureAwait(true);
                ...
                Status = variant.SvgPath is not null

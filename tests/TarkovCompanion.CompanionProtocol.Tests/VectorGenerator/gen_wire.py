...
coordinate_normalized = {"mapId": "customs", "floorId": None, "coordinateSpace": "Normalized", "projectionVersion": "tarkov-dev-1", "x": 0.25, "y": None, "z": 0.75}
projection = {
...
marks = {"cursor": {"revision": rev(1), "lastChangeId": cmd(4)}, "marks": [{
    "markId": gid("50000000-0000-4000-8000-000000000001"), "revision": 1, "kind": "Waypoint", "scope": "PairedDevice", "authorDeviceId": TABLET,
    "coordinate": coordinate_normalized, "label": "Stash spot", "color": "#00AACC", "createdUtc": at(30), "updatedUtc": at(30), "expiresUtc": None}]}
capture = {"cursor": {"revision": rev(1), "lastChangeId": cmd(5)}, "activeIntent": {
    ...
    "captureSessionId": gid("70000000-0000-4000-8000-000000000001"), "purpose": "LootDecision", "initiatingDeviceId": TABLET,
    "initiatingSurface": "TabletLandscape", "requestedUtc": at(40), "expiresUtc": at(160), "status": "Armed",
...
state = {"authorityEpoch": EPOCH, "globalRevision": rev(7), "desktopDeviceId": DESKTOP, "deviceModes": device_modes, "workspace": workspace, "marks": marks, "captureIntent": capture}
...
def base(n, kind, issued=0, lifetime=60, preview=None):
    return {"type": kind, "commandId": cmd(n), "requestedRevision": rev(1), "issuedUtc": at(issued), "expiresUtc": at(issued + lifetime), "offlineQueuePreview": preview}
...
commands = {
    ...
    "upsert-mark": dict(base(12, "upsertMark"), markId=gid("50000000-0000-4000-8000-000000000002"), expectedMarkRevision=0,
                        mark={"kind": "Ping", "scope": "PairedDevice", "coordinate": coordinate_world, "label": None, "color": "#FF8800", "expiresUtc": None}),
    ...
    "request-capture-intent": dict(base(14, "requestCaptureIntent"), intentId=gid("60000000-0000-4000-8000-000000000002"), correlationId="capture-correlation-2",
                                   captureSessionId=gid("70000000-0000-4000-8000-000000000002"), purpose="AutoDetect",
...
def update(kind, aggregate, global_revision, change):
    return {"type": "canonicalUpdate", "update": {"type": kind, "authorityEpoch": EPOCH, "globalRevision": rev(global_revision), "changeId": change, "changedUtc": at(50), "state": aggregate}}
...
for name, command in commands.items():
    files["commands/%s.json" % name] = envelope(command)
...
files["server/canonical-update-device-modes.json"] = server(4, update("deviceMode", device_modes, 2, cmd(2)))

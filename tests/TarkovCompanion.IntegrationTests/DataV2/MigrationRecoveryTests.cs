...
public sealed class MigrationRecoveryTests
{
    ...
    public async Task LedgerHasPairedUpgradeAndRollbackFixturesAndFreshDatabaseAppliesAll()
    {
        ...
        Assert.Equal(16, SqliteMigrationLedger.Entries.Count);
        Assert.Equal("0016_restore_task_objective_items", SqliteMigrationLedger.Entries[^1].Id);
        ...
        Assert.Equal(16, await V2TestDatabase.ScalarAsync(database.Factory, "SELECT COUNT(*) FROM schema_migrations;"));
        ...
        {
            latestCommand.CommandText = latest;
            await latestCommand.ExecuteNonQueryAsync(TestContext.Current.CancellationToken);
            latestCommand.CommandText = "SELECT COUNT(*) FROM sqlite_master WHERE type = 'table' AND name = 'flea_market_settings';";
            Assert.Equal(0L, await latestCommand.ExecuteScalarAsync(TestContext.Current.CancellationToken));
        }
    ...
    private static long RollbackArtifactExpectedCount(string migrationId) => migrationId switch
    {
        ...
        "0011_v2_data_platform" or
        "0016_restore_task_objective_items" => 1L,
        _ => 0L,
    };

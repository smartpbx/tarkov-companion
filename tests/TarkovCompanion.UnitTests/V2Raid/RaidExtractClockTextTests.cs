...
public sealed class RaidExtractClockTextTests
{
    ...
    public void Known_time_names_its_basis(string clock, string detail, string expected) =>
        Assert.Equal(expected, RaidCockpitViewModel.DescribeExtractClock(true, clock, detail));

    [Fact]
    public void Unknown_time_says_how_to_supply_it() =>
        Assert.Equal(
            "Time left unknown · open extracts or set by hand",
            RaidCockpitViewModel.DescribeExtractClock(true, "14:03 elapsed", "open extracts or set by hand"));

    [Fact]
    public void No_raid_has_no_extract_clock_line() =>
        Assert.Empty(RaidCockpitViewModel.DescribeExtractClock(false, string.Empty, "open extracts or set by hand"));
}

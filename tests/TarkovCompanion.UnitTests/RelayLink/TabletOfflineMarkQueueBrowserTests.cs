...
/// <summary>#290: an unreachable-relay mark is queued visibly and applied once after reconnect.</summary>
[Collection(RelayAdminKeyCollection.Name)]
public sealed class TabletOfflineMarkQueueBrowserTests : RealBrowserTestHarness
{
    [RealBrowserFact]
    public async Task OfflineMarkIsQueuedAndSentOnceAfterReconnect()
    {
        if (!HasHeadlessBrowser()) return;
        ...
        var polling = Task.Run(async () =>
        {
            ...
            {
                try { await desktop.Bridge.PollOnceAsync(pollLoop.Token); }
        ...
        var scriptPath = Path.Combine(RepositoryRoot(), "scripts", "test-tablet-offline-mark-queue.cjs");
        ...
        startInfo.ArgumentList.Add("Offline tablet");
        ...
        try
        {
            ...
            Assert.Equal("Offline flank", mark.State.Label);
            Assert.Equal(1, disk.Marks.AddCount);
        }
        finally
        {
            pollLoop.Cancel();
    ...
    private static TabletMapSurface MinimalSurface(DateTimeOffset now) => new(
        1, "customs", "Customs", "default", "v1", new TabletMapPlan(0, 0, 1000, 1000), null, [], ["1F"],
        [new TabletMapLayer("landmarks", "Landmarks", 0, true)], [], new TabletMapView("1F", 500, 500, 1, null, null),
        null, null, now);

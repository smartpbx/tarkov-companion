...
/// <summary>
/// #584 remainder: a tablet's ping has to leave the tablet's own Marks list when it expires, not
/// only the map. The list is fed by the paired protocol's MarkAggregate, which never expired
/// anything before. Real tablet page, real headless browser, real in-process relay and desktop;
...
public sealed class TabletMarksListExpiryTests
{
    ...
    public async Task ATabletsPingLeavesItsMarksListWhenItExpires()
    {
        ...
        var scriptPath = Path.Combine(RepositoryRoot(), "scripts", "test-tablet-marks-list-expiry.cjs");
        ...
        try
        {
            ...
            await UntilAsync(() => ContainsLine(stdoutLog, stdoutGate, "PING_LISTED") || browserProcess.HasExited, "PING_LISTED");
            AssertNotExited(browserProcess, stdoutLog);
            var ping = Assert.Single(desktop.Authority.Snapshot.CanonicalState.Marks.Marks);
            Assert.NotNull(ping.ExpiresUtc);
            ...
            Assert.Empty(desktop.Authority.Snapshot.CanonicalState.Marks.Marks);
        }
        finally
        {
            pollLoop.Cancel();

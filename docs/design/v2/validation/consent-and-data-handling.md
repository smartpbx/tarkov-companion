...
| ID-to-person mapping | Yes | Private study folder, separate file | Deleted with the screener answers |
...
| Verbatim quotes | Only if agreed per quote | This repository, attributed to an ID | Permanent |
...
> the report is finished. The report is public inside the project's repository, so it refers to you
> only as a participant number, and it will only quote you word for word if you say that is okay.
>
...
> afterwards to remove your data; if the report is not yet finished, we remove everything from you,

...
| M-06 | Give every screened candidate a fixed deletion deadline: screening timestamp + 30 × 24 hours for a person not recruited, and session start + 180 × 24 hours for a recruited participant. Delete non-recruited contact, roster and request-code-verifier data at the former deadline even though no session exists. Delete a participant's raw recordings/notes at the earlier of 30 days after sign-off or their latter deadline; keep only the private ID/contact/request-code and participant-to-evidence indexes until that latter deadline, except the minimum case record for an on-time withdrawal, which expires no later than deadline + 7 days. A consented no-show has no evidence index and is deleted by the earlier of the screening deadline or scheduled session + 7 days. | Raw material has a short bounded life while the minimal indexes keep on-time withdrawal executable. A candidate who never has a session still has contact and request-code data, so their deletion cannot be tied to a nonexistent session. The seven-day completion window prevents a request received just before the fixed deadline from becoming unexecutable. Consent gives exact UTC limits and states that participant-specific removal cannot be honoured after them or erase public history. | Proposed |
...
| P-04 | Every capture source enters one desktop-ordered queue. While any capture is analysing or awaits an unknown/mismatch decision, every later arrival waits unread; duplicate, context and filename-position publication run only when its turn arrives. A clipboard arrival is admitted only with one bounded, process-owned transient payload; when that payload expires, its ordered entry visibly fails and requires a new paste before later arrivals continue. | Preserves one observable order across independent and stitched captures without letting a later result overtake an unresolved earlier capture. Files can be re-read at their turn; clipboard data cannot, so it must be transiently owned or visibly fail rather than become an unread, impossible-to-read entry. A renamed duplicate can carry different filename metadata, so position cannot be a pre-duplicate fast path. | #271, #283 | Proposed |

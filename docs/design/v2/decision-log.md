...
| M-06 | Delete raw recordings/notes at the earlier of 30 days after sign-off or session start + 180 days. Keep only the private ID/contact/request-code and participant-to-evidence indexes until the fixed session-start + 180-day withdrawal deadline, then verify their deletion. | Raw material has a short bounded life while the minimal indexes keep on-time withdrawal executable. Consent gives an exact UTC deadline and states that participant-specific removal cannot be honoured after it or erase public history. | Proposed |
...
| P-04 | Every capture source enters one desktop-ordered queue. While any capture is analysing or awaits an unknown/mismatch decision, every later arrival waits unread; duplicate and context checks run only when its turn arrives. | Preserves one observable order across independent and stitched captures without holding pixels or letting a later result overtake an unresolved earlier capture. | #271, #283 | Proposed |

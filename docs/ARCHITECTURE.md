...
**The room.** A group agrees one key. The relay receives it over the configured transport, hashes
it, and buckets members by that hash; it does not persist the plaintext. Members publish
themselves and are answered with everyone else in one exchange, so there is no subscription to
hold open and a companion that is not running shows nothing. A
...
Durable state lives outside the tree the updater replaces: `marks.json`, `rooms.json`, and
submitted bodies under `reports/`. Positions and pings are never written. The same directory
contains updater-owned install/refusal stamps and an immediate-update request, which backups
must not restore. The relay cannot start a systemd unit and must not be able to—asking it to
update writes a file that a `.path` unit watches, and the updater ships its own units inside the
archive so a fix to them reaches the box.

...
result's layer and every positioned result object to match the canonical snapshot exactly, then
joins typed entries to objects by stable object ID. Map-only and unresolved-floor entries stay in
the same paged accessible list with explicit `List only`, location, and floor states. They are not
assigned a point. The existing deterministic point clustering applies to positioned loot objects;

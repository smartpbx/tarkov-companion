...
| ABUSE-PAIRED-HANDSHAKE-AUTHORITY-INJECTION | Spoofing / elevation of privilege | ACT-12 | Future relay composition accepts an HTTP body or other unverified DTO as a completed `PairingAttempt` or `SessionResumeAttempt` and passes it to the issue-#278 registry. The registry correctly consumes a completed aggregate but cannot prove that #277 performed the desktop-signature, WebAuthn, transcript, nonce, origin, counter, and user-verification checks that make completion authoritative. | — | RISK-PAIRED-AUTH-COMPOSITION |
| ABUSE-PAIRED-RATE-PARTITION-SPOOF | Spoofing / denial of service | ACT-5 | A future endpoint accepts a caller-supplied pairing `sourceHash`, or derives it from an untrusted forwarded address. An attacker rotates the value to bypass a per-source limit or makes victims share a partition. `RelayPairingInvitations` and `RelayRateLimiter` intentionally accept an opaque hash and cannot identify the trustworthy request boundary by themselves. | — | RISK-PAIRED-AUTH-COMPOSITION |
| ABUSE-PAIRED-OWNER-RECOVERY-EXPOSURE | Elevation of privilege | ACT-5 | Future composition exposes `OwnerRecoveryProtector.CreateGrant` as an unauthenticated recovery route. A remote caller obtains the single-use ceremony material that is intended to re-establish the owner only after a trusted local/operator recovery decision. | — | RISK-PAIRED-AUTH-COMPOSITION |
| ABUSE-PAIRED-LIVE-BEARER-THEFT | Spoofing / information disclosure | ACT-9 or ACT-13 | Same-user malware, a lost unlocked tablet, or a same-origin compromise obtains a paired session cookie or API bearer while it is live. Digest-only relay persistence, CSRF, and scoped authorization limit storage and browser-request abuse but do not make a copied bearer proof-of-possession; it works inside that session's scope until rotation, revocation, or expiry. | — | RISK-GROUP-KEY-LOCAL-EXPOSURE |

...
| --- | --- | --- | --- | --- |
| A-1 | Player's own profile, raid, and quest history | Personal, local | Profile state (including profile-level progress collections) is JSON in `%LOCALAPPDATA%\TarkovCompanion\Config\profile.json`; raid history and additional quest-progress stores are in SQLite | `AppComposition.cs`; `JsonFilePlayerProfileService.cs`; `docs/DATABASE.md` |
...
| A-10 | Problem report bodies | Personal (describes a player's machine), redacted before send | `reports/*.md` on relay disk, readable only with the admin key. Kestrel's effective request-body ceiling is 32 KiB; files have no count, age, or disk-quota lifecycle today | `Program.cs`; `ProblemReports.cs`; `docs/OPERATIONS.md` |

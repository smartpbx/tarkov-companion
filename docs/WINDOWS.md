...
Path discovery checks ordinary uninstall registry values and conventional user folders, then returns only directories that exist. Partial discoveries carry reduced confidence rather than fabricated paths. Users can still configure paths manually at the application layer. Candidates cover the launcher and Steam uninstall keys, Steam libraries from `libraryfolders.vdf`, the launcher's folder names on every fixed drive, and Documents/Pictures including OneDrive redirection; they are built by `EftPathCandidateBuilder` from an `EftDiscoveryEnvironment`, so fixture trees test them off Windows. Until the logs, screenshots and game data are all found, the Raid page shows a readiness strip with one-tap fixes (#712 1-13).

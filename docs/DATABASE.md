...
- `0019_loot_scan_history.sql` keeps each completed Loot Scan (its calls as shown, value, confidence, the recommendation ruleset version, raid and map; no pixels) in `loot_scans`. The store keeps the newest 500 and none older than 90 days on every save; the maintenance prune also removes scans older than the retention cutoff, and a raid's hard delete removes its scans.
- `0020_correction_memory.sql` keeps what the player taught by correcting a read (#712 1-12), on this PC only and never in an export or report: `learned_icon_references` (a named cell's icon squares as PNG, the newest 8 per item and 1,000 in all; never the screen), `learned_text_aliases` (an OCR reading picked as another item, used once picked twice) and `learned_frame_corrections` (a corrected cell or flea item by the frame's content hash, so the same frame read again stays corrected). Setup › Game & Capture counts them and deletes them.
...
`WriteFaultInjectionTests` aborts the Nth write to one table from a SQLite trigger, mid-transaction, in unmodified production code: a catalog republication (at an item, a category link or a price row) keeps every row, search entry and head of the published catalog; an outbox batch admits no item and claims no sequence; a raid-history operation leaves no raid, event or ledger row and applies once on retry; a purge failing on its second raid keeps all three. Each retry succeeds unchanged.

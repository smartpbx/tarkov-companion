...
- The items payload also carries the flea market's two listing rates. An items refresh writes them to the single row of `flea_market_settings` (migration 0014) with the time they were observed; a payload without usable rates leaves the last row in place, and its date is what lets a reader refuse it once it is old. `SqliteItemMarketFactSource` reads them, together with `items.base_price` and the listing count and trader buy offers kept in `items.raw_json`.

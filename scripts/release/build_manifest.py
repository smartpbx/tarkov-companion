...
VERIFICATION_METADATA = ("update.json", "SHA256SUMS.txt", "GROUPSERVER-SHA256SUMS.txt", "VELOPACK-SHA256SUMS.txt")
...
def inspect_desktop(archive_path: Path, source_root: Path, version: str, commit: str) -> tuple[dict[str, str], list[dict[str, Any]]]:
    ...
    with zipfile.ZipFile(archive_path) as archive:
        ...
        build_info = parse_build_info(archive.read("BUILD_INFO.txt"))
        require_informational_version(archive.read(DESKTOP_ASSEMBLY), identity, f"desktop {DESKTOP_ASSEMBLY}")
        notices = (source_root / "docs/THIRD_PARTY_NOTICES.md").read_bytes()
        if normalize_text(archive.read("THIRD_PARTY_NOTICES.md")) != normalize_text(notices):
        ...
        if read_json_bytes(archive.read("THIRD_PARTY_INVENTORY.json"), "packaged inventory") != inventory:
        ...
        for member in archive.getmembers():
            ...
            if name in (RELAY_EXECUTABLE, RELAY_ASSEMBLY) or name.lower().endswith(".dll") or reviewed:
                ...
                if reviewed:
                    ...
                    expected = (source_root / "deploy/group-server" / name.removeprefix("deploy/")).read_bytes()
                    if value != expected:
...
def nuspec_identity(path: Path) -> tuple[str, str]:
    try:
        with zipfile.ZipFile(path) as archive:
            ...
            document = ElementTree.fromstring(archive.read(nuspecs[0]))
...
def inspect_velopack(payload: Path, sums: dict[str, str], version: str) -> None:
    ...
    if "RELEASES" in sums:
        for line in (payload / "RELEASES").read_text(encoding="utf-8-sig").splitlines():
            parts = line.split()
...
def reconcile(payload: Path, source_root: Path, commit: str, verification_run_id: str) -> dict[str, Any]:
    ...
    accounted = set(VERIFICATION_METADATA) | set(PUBLISHER_FILES) | set(portable) | set(relay) | set(velopack)
    unexpected = sorted(path.name for path in payload.iterdir() if path.name not in accounted and not NOT_ARTIFACTS.match(path.name))
    if unexpected:
...
def create_manifest(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for name, source in (("THIRD_PARTY_NOTICES.md", "docs/THIRD_PARTY_NOTICES.md"),
        ...
        if not (payload / name).is_file() or (payload / name).read_bytes() != (args.source_root / source).read_bytes():
    ...
    return {
        ...
        # pause, rollback and last-known-good cannot apply to one kind and not the others. No
        # data or model artifact is released yet; the empty lists say so rather than implying one.
        "feeds": {
            "binary": {"authentication": "required", "artifacts": [item["name"] for item in artifacts if item["component"] != "release"]},
            "data": {"authentication": "required", "artifacts": []},
            "model": {"authentication": "required", "artifacts": []},
        },
    }
...
def extract_sbom_input(payload: Path, output: Path, identity: dict[str, Any]) -> None:
    ...
    with zipfile.ZipFile(payload / identity["desktopArchive"]) as archive:
    ...
    with tarfile.open(payload / identity["relayArchive"], "r:gz") as archive:
        members = archive.getmembers()
        for member in members:
            if not safe_member(member.name) or not (member.isfile() or member.isdir()):
                raise ManifestError(f"relay archive contains an unsafe entry: {member.name}")
        # Every member has already been restricted to a relative file or directory, which is what
        # the data filter enforces; the filter is still used wherever this Python provides it.
        if hasattr(tarfile, "data_filter"):
            archive.extractall(relay, members=members, filter="data")
        else:
            archive.extractall(relay, members=members)
...
def main() -> int:
    ...
    except (ManifestError, OSError, tarfile.TarError, zipfile.BadZipFile) as exception:

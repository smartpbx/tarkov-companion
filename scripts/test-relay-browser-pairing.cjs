...
async function main() {
  ...
  try {
    ...
    // relay, the same as production.
    await page.locator("#pairingDone").waitFor({ state: "visible", timeout: 30000 });
    ...
    await page.locator("#pairingDone").waitFor({ state: "visible", timeout: 15000 });
